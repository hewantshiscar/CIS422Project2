...
class User:
	...
	def __init__(self, user_type, first, last, age, gender, questionnaire, bio, email, username, password):
	 	...
	 	self.user_compats = ["", "", "", "", ""] # User Matches corresponding %
...
def compat():
	...
	if current_user.user_type:
		...
		for user in current_user.user_matches:
			...
			for i in range(4, 7):
				higher(current_user, user, i, count)
			...
			for i in range(7, 15):
				equal_or_higher(current_user, user, i, count)

			# 15) How much do you value integrity?
			equal_q_answer(current_user, user, 15, count)

			# 16) How patient are you? (1 - 5)
			equal_or_higher(current_user, user, 16, count)

			# 17) How social are you? (1 - 5)
			# 18) What is your learning style? (1 - 5)
			# 19) Career goals (1 - 5)
			for i in range(17, 20):
				equal_q_answer(current_user, user, 19, count)
			...
			for i in range(20, 22):
				equal_q_answer(current_user, user, i, count)
		...
		for user in current_user.user_matches:
			...
			# 6) How good are your organizational skills? (1 - 5)
			for i in range(4, 7):
				lower(current_user, user, i,count)
			...
			# 14) Professionalism (1 - 5)
			for i in range(7, 15):
				equal_or_lower(current_user, user, i, count)

			# 15) How much do you value integrity?
			equal_q_answer(current_user, user, 15, count)
			
			# 16) How patient are you? (1 - 5)
			equal_or_lower(current_user, user, 16, count)

			# 17) How social are you? (1 - 5)
			# 18) What is your learning style? (1 - 5)
			# 19) Career goals (1 - 5)
			for i in range(17, 20):
				equal_q_lower(current_user, user, 19, count)
